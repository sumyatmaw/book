...
// ==========================================================
// 1. BACKEND LOGIC: DATABASE TABLE UPDATE
// ==========================================================
$message = "";
$message_type = "";
...
<head>
    ...
    <style>
        /* Compact and Modern Custom Scrollbar Design matching Dashboard */
        ::-webkit-scrollbar {
            width: 5px;
            height: 5px;
        }
...
<div class="flex h-screen overflow-hidden">
    ...
    <main class="flex-1 overflow-y-auto p-4 sm:p-6 lg:p-8 min-w-0 max-w-[1600px] w-full mx-auto">
        <div class="max-w-6xl space-y-6 sm:space-y-8">
            ...
            <!-- Alert Message Notification Banner (Close Button ပါဝင်သည်) -->
            <?php if (!empty($message)): ?>
                <div id="alertBanner" class="p-4 rounded-xl flex items-center justify-between shadow-sm border <?= $message_type === 'success' ? 'bg-emerald-50 border-emerald-200 text-emerald-800' : 'bg-rose-50 border-rose-200 text-rose-800' ?> transition-all duration-300">
                    ...
                    <button onclick="closeAlertBanner()" class="text-slate-400 hover:text-slate-600 p-1 rounded-lg hover:bg-black/5 transition cursor-pointer">
            ...
            <div class="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-4 bg-white p-4 sm:p-5 rounded-2xl border border-slate-200/60 shadow-sm">
                ...
                <div class="flex flex-wrap sm:flex-nowrap items-center gap-3">
                    <?php if (strtolower($order['status'] ?? 'pending') === 'pending'): ?>
                        ...
                        <form method="POST" onsubmit="return confirm('Do you want to mark this order as completed?');" class="w-full sm:w-auto">
            ...
            <div class="bg-white p-5 sm:p-6 shadow-md rounded-2xl border border-gray-100">
                ...
                <?php if ($payment): ?>
                    <div class="grid grid-cols-1 md:grid-cols-2 gap-6 text-sm text-gray-600">
                        ...
                        <!-- Safe Slip Loader Utility (Pop-up Modal ပြုလုပ်ထားသည်) -->
                        <div class="flex flex-col items-start md:items-center justify-center">
                            ...
                            <?php if (!empty($payment['payment_slip'])): ?>
                                <div onclick="openSlipModal(this.querySelector('img').src)" id="slipContainer" class="block relative group cursor-pointer">
                                   <img src="<?= $actual_slip_path; ?>" 
                                        onerror="handleSlipError(this, '<?= htmlspecialchars($payment['payment_slip']); ?>')" 
                                        class="w-40 sm:w-48 h-auto object-cover rounded-xl border border-slate-200 shadow-sm hover:opacity-90 transition" 
                                        alt="Payment Slip" />
                                </div>
...
<div id="imageModal" class="fixed inset-0 bg-black/80 backdrop-blur-sm z-50 hidden flex items-center justify-center p-4 transition-opacity duration-300 opacity-0" onclick="closeSlipModal()">
    <div class="relative max-w-4xl max-h-[90vh] bg-white rounded-2xl overflow-hidden shadow-2xl p-2 flex flex-col items-center justify-center" onclick="event.stopPropagation()">
        
        <!-- Modal Close Button (X) -->
        <button onclick="closeSlipModal()" class="absolute top-3 right-3 text-slate-500 hover:text-slate-800 bg-slate-100 hover:bg-slate-200 rounded-full w-9 h-9 flex items-center justify-center transition focus:outline-none z-10 cursor-pointer shadow-sm">
            <i class="fa-solid fa-xmark text-lg"></i>
        </button>

        <!-- Preview Image Container -->
        <div class="overflow-auto max-h-[85vh] p-2 flex justify-center items-center">
            <img id="modalImage" src="" alt="Payment Slip Preview" class="max-w-full max-h-[80vh] object-contain rounded-lg">
        </div>
    </div>
</div>
...
<script>
    ...
    // Alert Message Banner ပိတ်ရန် Function
    ...
    // Modal Operations (Payment Slip ပွင့်/ပိတ် ပြုလုပ်ရန်)
    function openSlipModal(imgSrc) {
        const modal = document.getElementById('imageModal');
        const modalImg = document.getElementById('modalImage');
        
        if (imgSrc && !imgSrc.includes('Image+Not+Found')) {
            modalImg.src = imgSrc;
            modal.classList.remove('hidden');
            setTimeout(() => {
                modal.classList.remove('opacity-0');
            }, 10);
            document.body.style.overflow = 'hidden'; // Background scroll ခဏပိတ်ရန်
        }
    }
    ...
    function closeSlipModal() {
        ...
        setTimeout(() => {
            ...
            document.body.style.overflow = ''; // Background scroll ပြန်ဖွင့်ရန်
        }, 300);
    }

    // ESC Key နှိပ်ပါက မော်ဒယ် ပိတ်ပေးရန်
    ...
    function handleSlipError(imgElement, filename) {
        if (!imgElement.getAttribute('data-tried-admin')) {
            ...
            if (slipContainer) {
                slipContainer.removeAttribute('onclick');
                slipContainer.classList.remove('cursor-pointer');
            }
        }
    }
</script>
